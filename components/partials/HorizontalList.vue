    ...
    <div class="vhl-container" :style="_style.container">
      <div class="vhl-list" ref="list" :class="_options.list.class" :style="_style.list">
        <div v-for="item in items" ref="item" class="vhl-item" :class="_options.item.class" :style="_style.item">
          <a  @click.prevent="showLarge(item)">
...
export default {
  ...
  computed: {
    _options() {
      ...
      return {
        navigation: {
          start: options.navigation.start ? options.navigation.start : 992,
          color: options.navigation.color ? options.navigation.color : '#000',
        },
        item: {
          class: options.item ? options.item.class : '',
          padding: options.item ? options.item.padding : 16,
        },
        list: {
          class: options.list.class ? options.list.class : '',
          windowed: options.list.windowed ? options.list.windowed : 1200,
          padding: options.list.padding ? options.list.padding : 24,
        },
        responsive: [
  ...
  methods: {
    ...
    showLarge(picture) {
      this.largePicture = {src: picture.image, alt: picture.title}
